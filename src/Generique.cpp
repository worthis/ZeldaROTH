...
void Generique::initDisclamer()
{
    ...
    dst.x = 0;
    dst.y = 0;
    SDL_BlitSurface(attention, NULL, image, &dst);
...
void Generique::initSelection()
{
    ...
    gpJeu->affiche(image, "НАСТРОЙКИ", 60, 200);
    gpJeu->affiche(image, "РЕКОРДЫ", 215, 200);
...
void Generique::initOption()
{
    ...
    gpJeu->affiche(image, "НАСТРОЙКИ", 38, 15);
    gpJeu->affiche(image, "МУЗЫКА", 60, 71);
    gpJeu->affiche(image, "ЗВУКИ", 60, 135);
    gpJeu->affiche(image, "ВЕРНУТЬСЯ", 60, 200);
...
void Generique::initRecord()
{
    ...
    gpJeu->affiche(image, "РЕКОРДЫ", 40, 15);
    gpJeu->affiche(image, "ЛУЧШЕЕ ВРЕМЯ:  " + oss.str(), 140, 16);
    gpJeu->affiche(image, "100% РАНГ", 44, 56);
    gpJeu->affiche(image, "ВЫСШИЙ РАНГ", 44, 104);
    gpJeu->affiche(image, "РАНГ СКОРОСТИ", 44, 152);
    gpJeu->affiche(image, "ВЕРНУТЬСЯ", 63, 200);
    gpJeu->affiche(image, "ОЧИСТИТЬ", 215, 200);
...
{
    cadre(104, 84, 112, 72);
    gpJeu->affiche(image, "ОЧИСТИТЬ ?", 133, 93);
    gpJeu->affiche(image, "ДА", 151, 113);
    gpJeu->affiche(image, "НЕТ", 151, 133);
}

void Generique::initCharger()
{
    cadre(104, 84, 112, 72);
    gpJeu->affiche(image, "ЗАГРУЗИТЬ", 139, 93);
    gpJeu->affiche(image, "УДАЛИТЬ", 139, 113);
    gpJeu->affiche(image, "ОТМЕНА", 139, 133);
}
...
void Generique::initAide1()
{
    ...
    {
        gpJeu->affiche(image, "Таскать предметы без выбора перчаток: B", 24, ligne);
        ligne += 16;
    }
    if (ligne >= 176)
        return;
...
void Generique::initRang(int i)
{
    ...
    case 0:
        ...
        if (gpJeu->getKeyboard()->getRang(i))
            gpJeu->affiche(image, "РАНГ: Герой Хайрула", 96 - 2 - 16, 104);
        else
            gpJeu->affiche(image, "РАНГ: НЕИЗВЕСТЕН", 96 - 2 - 16, 104);
        gpJeu->affiche(image, "Завершить игру на 100%", 96 - 2 - 16, 120);
        break;
    case 1:
        ...
        l = 72;
        if (gpJeu->getKeyboard()->getRang(i))
            gpJeu->affiche(image, "РАНГ: Шикардос", 72 - 8 - 16, l);
        else
            gpJeu->affiche(image, "РАНГ: НЕИЗВЕСТЕН", 72 - 8 - 16, l);
        l += 16;
        ...
        l += 16;
        gpJeu->affiche(image, "не покупая бутылки", 72 - 8 - 16, l);
        l += 16;
        gpJeu->affiche(image, "и не подбирая Щит.", 72 - 8 - 16, l);
        l += 16;
        break;
    case 2:
        cadre(71 - 10, 88, 178 + 20, 64);
        if (gpJeu->getKeyboard()->getRang(i))
            gpJeu->affiche(image, "РАНГ: Марафонец Хайрула", 78 - 10, 96);
        else
            gpJeu->affiche(image, "РАНГ: НЕИЗВЕСТЕН", 78 - 10, 96);
        gpJeu->affiche(image, "Завершить игру менее, чем за", 78 - 10, 112);
        gpJeu->affiche(image, "два часа", 78 - 10, 128);
        break;
    }
}
...
void Generique::initScore()
{
    ...
    oss << tmp;
    if (tmp == 1)
        gpJeu->affiche(image, "Погиб лишь раз", 10, 50);
    else
    ...
    oss << tmp;
    pctg += tmp;
    gpJeu->affiche(image, ("Частиц сердца: " + oss.str() + " / 36").c_str(), 10, 70);
    ...
    if (gpJoueur->getEpee())
        tmp++;
    if (gpJoueur->getEpee() > 1)
        tmp++;
    if (gpJoueur->getEpee() > 3)
        tmp++;
    ...
    oss << tmp;
    pctg += tmp;
    gpJeu->affiche(image, ("Предметов: " + oss.str() + " / 56").c_str(), 10, 90);
    ...
    if (gpJoueur->getMort() == 0 &&
        gpJoueur->hasBouteille(0) == 0 &&
        gpJoueur->hasBouteille(1) == 0 &&
        gpJoueur->hasBouteille(2) == 0 &&
        gpJoueur->hasMedaillon(0) == 0 &&
        gpJoueur->hasMedaillon(1) == 0 &&
        gpJoueur->hasMedaillon(2) == 0 &&
        gpJoueur->getVieMax() == 20 &&
        gpJoueur->getMagieMax() == 32 &&
        gpJoueur->getBouclier() == 0)
    {
        result = "Шикардос";
        gpJeu->affiche(image, " - " + result, 52, ligne);
        ligne += 20;
        gpJeu->getKeyboard()->setRang(1);
        gpJeu->getKeyboard()->saveP();
    }
    else
    {
        if (pctg == 100)
        {
            result = "Герой Хайрула";
            gpJeu->getKeyboard()->setRang(0);
            gpJeu->getKeyboard()->saveP();
        }
        if (pctg < 100)
    ...
    if (temps <= 7200)
    {
        result = "Марафонец Хайрула";
        gpJeu->getKeyboard()->setRang(2);
        gpJeu->getKeyboard()->saveP();
        gpJeu->affiche(image, " - " + result, 52, ligne);
        ligne += 20;
    }
...
{
    if (!imageNuit)
        imageNuit = IMG_Load("data/images/logos/nuit.png");
    SDL_Rect dst;
    dst.x = 0;
    dst.y = 0;
