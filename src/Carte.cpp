...
void Carte::init()
{
    ...
    if (gpJeu->isDehors())
    {
        ...
        dst.x = gpJoueur->getX() / 16;
        dst.y = gpJoueur->getY() / 16;
        dst.y += 30;
...
void Carte::initDonjon()
{
    ...
    initName();
    initItems();
    if (gpJoueur->getCle(gpJeu->getZone() - 12, 0))
        initEtage();
}
...
void Carte::initEtage()
{
    ...
    src.y = 0;
    dst.x = 25;
    dst.y = 98 - gpJeu->getMonde()->etage() * 16;
    SDL_BlitSurface(tmp, &src, image, &dst);
    SDL_FreeSurface(tmp);
...
void Carte::initItems()
{
    ...
    if (gpJoueur->getCle(gpJeu->getZone() - 12, 1))
    { // boussole
        src.h = 16;
        src.w = 16;
        src.x = 96;
    ...
    if (gpJoueur->getCle(gpJeu->getZone() - 12, 2))
    { // cle boss
        src.h = 16;
        src.w = 16;
        src.x = 16;
...
void Carte::initName()
{
    ...
    {
    case 0:
        gpJeu->affiche(image, "Лесной Храм", 128, 25);
        break;
    case 1:
        gpJeu->affiche(image, "Водный Храм", 128, 25);
        break;
    case 2:
        gpJeu->affiche(image, "Горный Храм", 128, 25);
        break;
    case 3:
        gpJeu->affiche(image, "Пустынный Храм", 128, 25);
        break;
    case 4:
        gpJeu->affiche(image, "Темный Храм", 128, 25);
        break;
    case 5:
        gpJeu->affiche(image, "Огненный Храм", 128, 25);
        break;
    case 6:
        gpJeu->affiche(image, "Ледяной Храм", 128, 25);
        break;
    case 7:
        gpJeu->affiche(image, "Тайный Храм", 128, 25);
        break;
    case 8:
        gpJeu->affiche(image, "Замок Хайрула", 128, 25);
        break;
    }
}
...
void Carte::moveLevel(Direction dir)
{
    if (!gpJeu->isDonjon())
        return;
    if (!gpJeu->getJoueur()->getCle(gpJeu->getZone() - 12, 0))
        return;
    if (gpJeu->getZone() != 13 && gpJeu->getZone() != 19)
