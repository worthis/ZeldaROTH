...
void Menu::drawCurseur(SDL_Surface *gpScreen)
{
    ...
    if (anim == 1)
    {
        if (gpJoueur->getTypeAnim() < 4 || gpJoueur->getTypeAnim() > 19)
            src.x = 48;
        else
            src.x = 80;
...
void Menu::drawStatut(SDL_Surface *gpScreen)
{
    ...
    if (vm < 10)
        oss << "0";
    oss << vm;
    gpJeu->affiche(gpScreen, "ЗДОРОВЬЕ", 152, 36 - dec + 8);
    gpJeu->affiche(gpScreen, ": " + oss.str(), 148 + 6 * 9, 36 - dec + 8);
...
void Menu::drawCristaux(SDL_Surface *gpScreen)
{
    ...
    gpJeu->affiche(gpScreen, "КРИСТАЛЛЫ:", 24, 180 + dec);
