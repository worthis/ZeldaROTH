...
void Statut::drawBoss(SDL_Surface *gpScreen)
{
    ...
    {
        if (jauge == true && valjauge > 0 && nivjauge > 0)
            nivjauge--;
        if (jauge == false && nivjauge < 50)
            nivjauge++;
...
void Statut::drawMagie(SDL_Surface *gpScreen)
{
    ...
    case 8:
        src.x = 32;
        src.y = 34;
        if (gpJoueur->hasObjet(O_GANTS) == 1)
            src.y = 85;
        break;
    default:
        src.x = 0;
        if (gpJoueur->hasBouteille(gpJoueur->getObjet() - 9) == 3)
            src.x = 16;
        if (gpJoueur->hasBouteille(gpJoueur->getObjet() - 9) == 4)
            src.x = 32;
        if (gpJoueur->hasBouteille(gpJoueur->getObjet() - 9) > 1)
            src.y = 51;
        if (gpJoueur->hasBouteille(gpJoueur->getObjet() - 9) == 1)
            src.y = 68;
        break;
